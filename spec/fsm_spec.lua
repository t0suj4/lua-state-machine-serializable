...
describe("Lua state machine framework", function()
    ...
    describe("a wildcard", function()
        it("should accept handler for wildcards", function()
            local fsm = machine.create{initial = "idle", events = {{ name = "go", from = "*", to = "busy"}}}
            assert.has.no.errors(function()
                fsm.onleaveidle = stub.new()
            end)
        end)
    end)
end)
